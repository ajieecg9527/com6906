...
def infer(cfg: DictConfig) -> None:
    ...
    train_msbg_csv = sb.dataio.dataio.load_data_csv(train_exp_dir / f"cpc2_asr_data/{cfg.dataset.train_set}.train.msbg.csv")
    ...
    # Load haspi scores
    train_haspi_file = Path(hparams["haspi_folder"]) / f"{cfg.dataset.train_set}.haspi.jsonl"
    train_haspi_records = read_jsonl(train_haspi_file)
    train_ear_choices = {haspi_record["signal"]: haspi_record["ear"] for haspi_record in train_haspi_records}
    ...
    # Compute the similarity on the train set
    train_enc_similarity, train_dec_similarity = {}, {}
    for i, record in tqdm(train_msbg_csv.items()):
        sig_msbg, wrd, sig_name = record["signal"], record["wrd"], record["signal_ID"]
        ear = train_ear_choices[sig_name]
        similarity = compute_similarity(sig_msbg, wrd, asr_model, bos_index, tokenizer, ear)
        train_enc_similarity[sig_name] = similarity[0].tolist()
        train_dec_similarity[sig_name] = similarity[1].tolist()

        with (train_exp_dir / "train_enc_similarity.json").open("w", encoding="utf-8") as fp:
            json.dump(train_enc_similarity, fp)
        with (train_exp_dir / "train_dec_similarity.json").open("w", encoding="utf-8") as fp:
            json.dump(train_dec_similarity, fp)
