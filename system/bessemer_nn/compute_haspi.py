""" This script computes the HASPI scores. """
...
from model.hearing_loss.index.haspi import compute_haspi_for_signal

logger = logging.getLogger(__name__)


@hydra.main(config_path=".", config_name="config")
def compute_haspi(cfg: DictConfig) -> None:
    ...
    # Compute the haspi scores and record the scores from the better ear
    ...
    results_file = Path(f"{cfg.train_path.exp_dir}") / f"{cfg.dataset.train_set}.haspi.jsonl"
    for record in tqdm(train_records):
        ...
        # Get haspi score and better ear
        haspi, ear = compute_haspi_for_signal(signal_name, cfg.train_path)

        # Write result
        result = {"signal": signal_name, "haspi": haspi, "ear": ear}
    ...
    results_file = Path(f"{cfg.test_path.exp_dir}") / f"{cfg.dataset.test_set}.haspi.jsonl"
    for record in tqdm(test_records):
        ...
        # Get haspi score and better ear
        haspi, ear = compute_haspi_for_signal(signal_name, cfg.test_path, "anechoic")
